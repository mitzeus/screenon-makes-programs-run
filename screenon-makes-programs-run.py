...
if sys.argv[1] == "--search":
    ...
    mustMonitorBeAlone = input(f"Do you want this screen: \"{chosenMonitorName}\" to be the only enabled screen in order to trigger a reaction? (y/n): ")
    ...
    addFlags = input("Do you want to add any arguments to start the file with? (eg. -f, -bigpicture) (y/n): ")
    if addFlags.lower() in ["y", "yes", "yeah", "sure"]:
        flags = input("Please input your arguments as follows: \"-i -force -startminimized\": ")
    ...
    testAnswer = input("Do you want to test the path and arguments to make sure it's working correctly? This will try to start selected program. (y/n): ")
    if testAnswer.lower() in ["y", "yes", "yeah", "sure"]:
        ...
        while testWorked == False:
            ...
            testAnswer = input("If the program did not work and you want to change path or arguments and test again, type \"test\", otherwise press any key..: ")
            if testAnswer.lower() == "test":
                ...
                flags = input("Please re-input your arguments as follows: \"-i -force -startminimized\" (keep empty if no arguments should be used): ")
            else:
                testWorked = True
    ...
    if submitAnswer.lower() in ["y", "yes", "yeah", "sure"]:
        ...
        if not os.path.exists("C:/custom-scripts/screenon-makes-programs-run/"):
            try:
                ...
                displayError("❌ Could not create script directory, please make sure you have permission to write to your C:\\ drive")

        try:
            shutil.move('config.json', 'C:/custom-scripts/screenon-makes-programs-run/config.json')
            print("✅ Moved config to script file.")
        except:
            displayError("❌ Could not move config to script folder, please make sure you have permission to write to your C:\\ drive")

        try:
            shutil.copyfile("screenon-makes-programs-run.py", "C:/custom-scripts/screenon-makes-programs-run/script.py")
            print("✅ Copied script to script file")
        except:
            displayError("❌ Could not copy script to script folder, please make sure you have permission to write to your C:\\ drive")
    ...
    if config['mustMonitorBeAlone'] == True:
        if finalMonitorArray[0][1] == config['chosenScreen'] and amountOfMonitorsTotal == 1:
